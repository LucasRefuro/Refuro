...
(async()=>{
  ...
  const u=(w.__fetch.find(x=>/klantportaal$/.test(x[0]) && x[1].actie==='uitnodigen')||[])[1]||{};
  const m=w.__fetch.filter(x=>/klantportaal$/.test(x[0]) && x[1].actie==='melding').map(x=>x[1].soort);
  ok('mail naar de klant bij aannemen, nieuwe opdracht en stappen', m.includes('aangenomen') && m.some(x=>['opgehaald','gewist','getest','afgerond'].includes(x)), JSON.stringify(m));
